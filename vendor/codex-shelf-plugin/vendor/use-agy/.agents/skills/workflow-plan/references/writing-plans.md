...
packages/core/
  - Modify: src/Modules/Widgets/Service.ts — add new widget type
  - Create: src/Modules/Widgets/Domain.ts — widget domain model
  - Test:   test/Widgets/Service.test.ts

packages/cli/
  - Create: src/commands/widget.ts — new CLI command
...
- [ ] **Exact file paths.** Follow the repo's conventions (see `AGENTS.md`): e.g. PascalCase for Effect module files (`Modules/Users/Domain.ts`), tests in `test/` directories.
...
- Path aliases: `@/*` for local imports, `@app/*` for cross-package imports
- Import from files, not barrels (import the specific module, not a package index)
- No file extensions in imports (no `.ts`, `.tsx`, `.js`)
- `Schema.TaggedError` with `_tag` for tagged errors
- Tests in `test/` directories, not co-located
- Named exports over default exports
- No type casting (`as`, `as any`, etc.) — fix the underlying type
- Use idiomatic Effect patterns (`Effect.gen`, `pipe`, `Layer`, `Service`, `Schema`)

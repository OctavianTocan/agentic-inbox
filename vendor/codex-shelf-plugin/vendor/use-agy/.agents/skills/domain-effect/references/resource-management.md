...
const processFile = (path: string) =>
  Effect.acquireUseRelease(
    Effect.succeed({ path, fd: 1 }),
    (handle) => Effect.succeed(`Content of ${handle.path}`),
    (handle, exit) => Console.log(`Closed ${handle.path}`)
  )
```
...
const createBucket = Effect.acquireRelease(
  ...
  (_, exit) => Exit.isFailure(exit) ? Console.log("Rollback bucket") : Effect.void
)
...
const makeTempFile = (prefix: string) =>
  Effect.acquireRelease(
    Effect.gen(function* () {
      ...
      return {
        path,
        write: (data: string) => Effect.sync(() => { contents = data }),
...
async function unsafe() {
  ...
  try { return await use(conn) }
  finally { await close(conn) } // May not run!
}

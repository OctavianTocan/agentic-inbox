...
Services in this project are defined with `Effect.Service` (see SKILL.md). The canonical way to mock them in tests is `makeTestLayer` from `@tooling/testing/test-layer` — see [effect-testing.md](effect-testing.md) for the project-specific helpers. This section covers the underlying `Layer` primitives.
...
class UserRepo extends Effect.Service<UserRepo>()('UserRepo', {
  effect: Effect.gen(function* () {
    return {
      findById: Effect.fn('UserRepo.findById')(function* (_id: string) {
        return Option.none<User>()
      }),
      save: Effect.fn('UserRepo.save')(function* (_user: User) {
        return undefined
      }),
    }
  }),
...
// Canonical project pattern: only implement the methods your test needs.
// Unimplemented methods die at runtime with service + method name.
const UserRepoTest = makeTestLayer(UserRepo)({
  findById: () => Effect.succeed(Option.some({ id: 'u1', name: 'Test User' })),
})

// Low-level alternative: full implementation via Layer.succeed.
const UserRepoLive = Layer.succeed(
  UserRepo,
  UserRepo.of({
    findById: () => Effect.succeed(Option.some({ id: 'u1', name: 'Test User' })),
    save: () => Effect.void,
...
```typescript
// Merge peer layers (no provider/consumer relationship)
const TestEnv = Layer.mergeAll(UserRepoTest, AuthenticationTest)
...
When the mock needs to track calls or evolve state across invocations, use `Layer.effect` with a `Ref`:
  ...
  Effect.gen(function* () {
    ...
    return Counter.of({
      increment: Effect.fn('Counter.increment')(function* () {
        return yield* Ref.updateAndGet(state, (n) => n + 1)
      }),
      get: Effect.fn('Counter.get')(function* () {
...
```typescript
const TestLayer = Layer.succeed(MyService, { fetch: () => Effect.succeed("data") })
...
it.effect("tests debounced stream", () =>
  Effect.gen(function* () {
    ...
    const stream = Stream.fromQueue(queue).pipe(
      ...
      Stream.runForEach((term) =>
        Effect.sync(() => results.push(term))
      )
    )
...
const ProcessedString = Schema.String.pipe(
  ...
  Schema.transform(
    Schema.String,
    { decode: (s) => s.toUpperCase(), encode: (s) => s.toLowerCase() }
  ),
...
const testLayer = MyService.DefaultWithoutDependencies.pipe(
  Layer.provide(makeTestLayer(MyRepo)({ /* mocks */ })),
  Layer.provide(ExternalClient.Test),
)
...
class Crypto extends Effect.Service<Crypto>()('Crypto', {
  ...
  static Test = Layer.succeed(Crypto, new Crypto({
    generateApiKey: Effect.succeed('app_test_key_000000'),
    hashApiKey: (key) => Effect.sync(() => createHash('sha256').update(key).digest('hex')),
    redactedKey: (key) => `app_${key.slice(4, 7)}...${key.slice(-3)}`,
  }))
}
```
...
const AuthTest = Layer.succeed(Authentication, Authentication.of({
...
const response = await handler(new Request('http://localhost/v1/my-endpoint', {
  method: 'GET',
...
The server app's `test/_shared/Setup.ts` exports shared auth constants — no factory functions, just reusable objects:
...
| `TestDatabaseLive`, `runEffect` | `@test/_shared/TestDatabase` | Real Postgres SQL client layers, convenience runner |
| `makeIntegrationSuite(options?)` | `@test/integration/Setup` | Integration harness: `{ auth, setup, teardown, runEffect }` |
| `setupE2ESuite(options)` | `@test/integration/Helpers` | E2E harness: handler + auth + HTTP helpers |
| `makeRequest`, `expectStatus`, `expectJson`, `expectError` | `@test/integration/Helpers` | HTTP request builder and response assertions |
...
const makeServiceLayer = (overrides = {}) =>
  MyService.DefaultWithoutDependencies.pipe(
    Layer.provide(makeTestLayer(MyRepo)({
...
describe('MySchema', () => {
  it('decodes valid data', () => {
    const result = Schema.decodeUnknownSync(MySchema)({ id: '1', name: 'test' })
...
const makeHandler = (serviceOverrides = {}) => {
  ...
  const HttpTestLive = HttpApiBuilder.group(TestApi, 'myEndpoints', (handlers) =>
    Effect.gen(function* () {
      const svc = yield* MyService
      return handlers.handle('list', () => svc.list())
    })

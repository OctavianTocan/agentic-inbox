...
class ToolkitConfig extends Effect.Service<ToolkitConfig>()(
  "ToolkitConfig",
  {
...
```typescript
class ToolkitLogger extends Effect.Service<ToolkitLogger>()(
  "ToolkitLogger",
  {
...
const LoggerLive = Layer.effect(Logger, Effect.gen(function* () {
  ...
  return { log: (msg) => Effect.sync(() => console.log(`[${config.port}] ${msg}`)) }
}))
...
const UserRepoMock = Layer.succeed(UserRepo, {
  findById: (id: string) => Effect.succeed(id === "1" ? { id: "1", name: "Test" } : null)
})
...
```typescript
// BAD
interface Bad { findById: (id: string) => Effect.Effect<User, Error, Logger | Database> }
// GOOD - clean interface, dependencies at layer construction
interface Good { findById: (id: string) => Effect.Effect<User, Error> }
```
...
class Bad extends Effect.Service<Bad>()("Bad", {
  effect: Effect.gen(function* () { const config = yield* Config; return {} })
}) {}
...
const good = Effect.scoped(Effect.gen(function* () {
  const conn = yield* db.connect()
  return yield* conn.query("...")
}))
```

...
- `packages/ci/skill-gen/src/**` — the only Effect-TS code in the repo today
- Any future Effect-based package added under `packages/**`
...
```bash
find packages/ci/skill-gen/src -name "*.ts"
grep -rl 'from "effect"' packages --include="*.ts" | grep -v vendor
```

## Audit Approach

Spawn one **sub-agent following `AGENTS.md` > Subagents per Effect package** (currently just `@ci/skill-gen`). Each agent:

...
Stream.make(1, 2, 3).pipe(
  Stream.run(
    Sink.forEach((n) => Console.log(`Processing: ${n}`))
  )
)
...
const stringSum = Sink.sum.pipe(
  Sink.mapInput((s: string) => parseInt(s, 10))
)
...
Stream.make(1, 2, 3, 4, 5).pipe(
  Stream.run(
    Sink.take<number>(3).pipe(Sink.collectLeftover)
  )
)
...
Stream.make(1, 2, 3, 4, 5).pipe(
  Stream.run(
    Sink.take<number>(3).pipe(Sink.ignoreLeftover)
  )
)
// Chunk([1, 2, 3])
```

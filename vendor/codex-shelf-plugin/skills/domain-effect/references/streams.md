...
const flat = stream.pipe(
  Stream.flatMap((id) => fetchRelatedItems(id))
)
...
await Effect.runPromise(
  stream.pipe(Stream.runForEach((n) => Console.log(n)))
)
...
const fileStream = Stream.acquireRelease(
  ...
  Stream.flatMap((fd) => readFromFd(fd))
)
...
```typescript
const [errors, successes] = yield* Stream.partitionEither(
  results,
  (result) =>
    result.ok

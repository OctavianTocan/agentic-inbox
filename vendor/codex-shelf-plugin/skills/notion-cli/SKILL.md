...
- **Headless / VPS (no OS keychain):** by default `ntn` stores/reads the token
  in the OS keychain, which is absent on a bare server — so commands fail with
  *"No auth token found for the current workspace"* even when a token is configured.
  Set **`NOTION_KEYRING=0`** to use the file-based token at `~/.config/notion/auth.json`
  instead, e.g. `NOTION_KEYRING=0 ntn pages create ...`. Diagnose with `ntn doctor`
  (it reports `Token valid` once `NOTION_KEYRING=0` is set). Prefer this over
  `ntn login` on any box where you can't open a browser.

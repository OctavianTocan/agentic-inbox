...
{{REPO_NAME}} is a **Bun + TypeScript monorepo** built on **Effect v4** (the
`effect-smol` line). It is a Bun workspace: shared tooling lives in `packages/`,
agent skills and commands live under `.agents/`. This file is the day-to-day
operating contract; the skills under `.agents/skills/` own the detailed
conventions.
...
- **Gate on exit codes:** Run `bun run ci` before claiming work complete; gate on its exit code, never on grepped output.
...
.agents/
  skills/                 Agent skills (hand-written + generated)
    skill-gen/            Generated from packages/ci/skill-gen
    workflow-gen/         Generated from packages/ci/workflow-gen
    ...
    workflow-plan/        Planning before implementation
    meta-housekeeping/    Repo-wide audits and maintenance
...
apps/                     Runtime services (HTTP API, workers, …) when needed
.github/
...
New packages follow the conventions the skills describe: Bun runtime, Effect v4
services, and a package-level `test/` directory.
...
- **Bucket then dispatch:** Decompose non-trivial work into independent buckets, one subagent per bucket. Many small focused buckets beat one monolith: parallelizes execution, keeps each context tight, isolates failures.
- **Brief them fully:** Subagents start with fresh context. Every prompt needs a full overview, explicit step-by-step instructions, and the skills they must activate.
...
- Title: keep the `type(scope): summary` prefix, then a plain imperative phrase. No PR/issue numbers, slice counts, diff stats, or emoji.
...
- Cut filler openers ("This PR…"), invented numbers, and AI tells ("comprehensive", "robust", "seamless").

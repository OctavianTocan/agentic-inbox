...
const decoded = Schema.decodeUnknownSync(Person)({ id: 1, name: "Bob", email: "bob@example.com" })
...
const fetchUser = Effect.fail(new HttpError({ status: 404, message: "User not found" }))
```
...
FastCheck.assert(
  FastCheck.property(personArbitrary, (person) => person.age >= 0 && person.age <= 120)
)
```
...
const RealisticPerson = Schema.Struct({
  ...
  email: Schema.String.annotations({
    arbitrary: () => (fc) => fc.constant(null).map(() => faker.internet.email())
  })
})
```
...
const UpdateUserRequest = Schema.partial(CreateUserRequest.pipe(Schema.omit("password")))

const PaginationParams = Schema.Struct({
  page: Schema.optional(Schema.NumberFromString.pipe(Schema.int(), Schema.positive()), { default: () => 1 }),
  limit: Schema.optional(Schema.NumberFromString.pipe(Schema.int(), Schema.between(1, 100)), { default: () => 20 })
})

const ErrorResponse = Schema.Struct({
  error: Schema.Struct({ code: Schema.String, message: Schema.String, details: Schema.optional(Schema.Unknown) })
})
```
...
export const Cents = Schema.Number.pipe(Schema.int(), Schema.nonNegative(), Schema.brand("Cents"))
```
...
| Annotation | Effect in generated OpenAPI |
|---|---|
| `identifier` | `$ref` name in components (`#/components/schemas/User`) |
| `title` | Human-readable name |
| `description` | Schema prose shown in docs |
| `examples` | Sample values |
| `default` | Default value |
| `documentation` | Long-form technical notes (distinct from `description`) |
...
// annotations on the schemas above flow through automatically

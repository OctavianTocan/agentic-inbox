...
Universal rules that apply to **all** domains. Every housekeeping subagent must enforce these alongside the domain-specific checklist.

`AGENTS.md` > Conventions owns the rule semantics and rationale (naming, parameters, JSDoc, immutability, function structure, file organization); read it for the full reasoning rather than relearning it here. This checklist keeps only the audit-specific enumeration: a stable `Q`-prefixed ID per rule plus a violation/correct example pair to grep and cite during a pass.
...
| 3 | Function JSDoc summary | Function, exported or helper, without a single-line summary | Add a concise `/** Describes intent. */` summary above the declaration |
...
| 12 | Public API JSDoc tags | Exported/public API has unclear parameters, return value, generics, or domain-required tags | Add `@param`, `@returns`, and `@template` only where they clarify the public contract or a domain skill requires them |
...
| 7 | No noisy private-helper tags | Private helper has boilerplate `@param`/`@returns` that restates TypeScript types | Keep the single-line summary; reserve tags for public APIs or domain-specific requirements |

...
**Effect tests:** poll an effect with a `waitUntil` test helper. It evaluates the effect until `predicate(value)` is true, then returns the value; it dies (raises a defect) on timeout, so callers never thread a typed timeout error.
...
A companion `waitForCompletion` helper awaits a `send` result's completion `Deferred` with a timeout; prefer it over sleeping after sending a message.

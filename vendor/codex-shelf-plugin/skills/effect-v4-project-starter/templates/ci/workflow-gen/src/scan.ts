...
const OPEN_TAG = /^\s*(?:\/\/|#)\s*<workflow-gen>\s*$/;
const CLOSE_TAG = /^\s*(?:\/\/|#)\s*<\/workflow-gen>\s*$/;
...
export interface RawFragment {
  /** Lines between the open/close tags, with comment prefixes still intact. */
  lines: string[];
...
/**
 * Collect every workflow-gen fragment found beneath a directory tree.
 *
 ...
 * @returns Fragment blocks sorted by relative path for deterministic merge order.
 */
export async function scan(
  ...
  const results = await Promise.all(
    filePaths.map((filePath) => processFile(filePath, baseDirResolved, verbose))
  );
...
async function processFile(
  ...
  if (!content.includes('<workflow-gen>')) {
    return [];
  }
...
function handleOpenTag(
  ...
  if (currentBlock !== null) {
    throw new Error(
      `Nested <workflow-gen> marker at ${filePath}:${lineNum}. Nested markers are not supported.`
    );
  }
  return [];
}
...
function handleCloseTag(
  ...
  if (currentBlock === null) {
    throw new Error(
      `Unexpected </workflow-gen> without opening tag at ${filePath}:${lineNum}`
    );
  }
...
function extractFragments(
  ...
  if (currentBlock !== null) {
    throw new Error(`Unclosed <workflow-gen> block in ${filePath}`);
  }

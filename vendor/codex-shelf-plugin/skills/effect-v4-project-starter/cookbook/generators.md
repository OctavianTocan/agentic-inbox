...
### 2. Copy The Bundled Generator Packages

The generators ship with this skill — copy them rather than rewriting them:

```bash
mkdir -p <project-root>/packages/ci
cp -R templates/ci/skill-gen     <project-root>/packages/ci/skill-gen
cp -R templates/ci/workflow-gen  <project-root>/packages/ci/workflow-gen
cp -R templates/ci/actions       <project-root>/packages/ci/actions
mkdir -p <project-root>/.github/actions
cp -R templates/github/actions/setup <project-root>/.github/actions/setup
```

These are normal Bun + Effect v4 CLI packages (`@ci/skill-gen`, `@ci/workflow-gen`) using `effect/unstable/cli`, `@effect/platform-bun`, `BunRuntime.runMain`, and `BunServices.layer`. Each carries a `SPEC.md` and an `e2e-test` fixture suite. `templates/ci/actions/{knip,lint,test,typecheck}` are co-located CI checkers whose `//<workflow-gen>` fragments define the `ci` workflow jobs.

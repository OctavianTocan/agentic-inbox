...
- SHOULD use optimistic updates for reversible user-initiated mutations where rollback is clear — show the result immediately, reconcile in the background
- SHOULD preload data for likely next actions when existing routing/data tools make it cheap (hover on a link → prefetch that page)
...
- MUST use the existing command palette when the surface already participates in global navigation. Add a new global command system only when the product need is explicit.

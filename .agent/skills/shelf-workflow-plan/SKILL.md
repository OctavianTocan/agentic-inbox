...
References: `references/writing-plans.md` (format conventions), `references/step-planner-prompt.md` (Phase 3 sub-agent prompt), `references/critic-prompt.md` (Phase 5 critic prompt), `../_shared/output-destination.md` (Phase 6 local/Notion/both gate).
...
Read `.context/plans/brainstorm.md`, relevant skill files (`domain-effect`, `domain-git`, `domain-cli`, `skill-gen`, …), `AGENTS.md`, and the key source files the brainstorm names.
...
**Output destination.** Before saving, resolve where the plan goes per `../_shared/output-destination.md` (ask once, then locked in for the flow; reuse `.context/output-target` if a prior skill set it). For `notion`/`both`, publish the plan to the Agent Plans workspace instead of, or in addition to, the committed repo copy.

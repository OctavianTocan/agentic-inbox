...
| [rules/api.md](rules/api.md) | `apps/api/**` (whole-entity repos; sub-modules; Schema SQL row decode) |
...
`.agent`, `.codex`, `.context`, `.cursor`, `.claude`, `.sentrux`, `.serena`, `.superpowers`, `vendor`, `node_modules`, `.codegraph`, Next.js `.next`, build outputs, lockfiles.

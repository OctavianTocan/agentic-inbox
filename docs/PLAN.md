...
**Policy = the `needsApproval` predicate** on mutating tools. Deterministic code: sensitive category (change_order, claim_dispute, safety, owner_escalation) OR dollar-amount/legal-keyword signals OR low confidence → approval required. **The keyword/dollar signals run against the RAW email subject AND body (deterministic, injection-resistant) — never against model output — so a prompt-injected email that manipulates its own classification still trips the gate, and subject-only signals (e.g. "INCIDENT REPORT" subjects) are covered.** Routine + confident → auto-execute. This is doubly enforced: the batch toolset also simply omits any ungated send (capability-based defense in depth, per Cloudflare reference). **Unit-tested invariant: sensitive input can never produce an auto-executed action.**
...
**Spike constraints (binding — proven against the live API, see docs/SPIKE-NOTES.md):**

...
type DynamicFragmentModule = {
  readonly getSkillFragments?: () =>
    | ReadonlyArray<DynamicSkillFragment>
    | Promise<ReadonlyArray<DynamicSkillFragment>>;
};

/**
 * Register TypeScript modules that export `getSkillFragments()` to contribute
 * skills without embedding markers in source files. Paths are relative to the
 * scan base directory (usually the repo root).
 *
 ...
 */
export async function loadDynamicFragments(
  baseDir: string,
  verbose = false
): Promise<ParsedSkillFragment[]> {
  ...
  for (const source of DYNAMIC_FRAGMENT_MODULES) {
    ...
    fragments.push(
      ...sourceFragments.map((fragment) => normalizeFragment(fragment, source))
    );

    if (verbose) {
      process.stdout.write(
        `dynamic-fragments: loaded ${sourceFragments.length} fragment(s) from ${source}\n`
      );
    }
  }
...
async function importDynamicModule(
  modulePath: string
): Promise<DynamicFragmentModule> {
  return (await import(
    pathToFileURL(modulePath).href
  )) as DynamicFragmentModule;
}
...
async function loadSourceFragments(
  ...
  if (!module.getSkillFragments) {
    throw new Error(
      `Dynamic skill source ${sourcePath} must export getSkillFragments().`
    );
  }
...
function normalizeFragment(
  fragment: DynamicSkillFragment,
  sourcePath: string
): ParsedSkillFragment {
  if (fragment.name.length === 0 || fragment.description.length === 0) {
    throw new Error(
      `Dynamic skill source ${sourcePath} returned a fragment without name or description.`
    );
  }

 ...
 * - other scalar keys: last writer wins (with warning)
 *
...
function mergeDefaultKey(
  ...
  if (key in target && target[key] !== value && verbose) {
    ...
    console.warn(
      `merge: conflicting value for '${key}' in workflow '${target.name}' from ${sourcePath}, using later value`
    );
  }
  target[key] = value;
}
...
/**
 * Union merge for `on` triggers.
 * Each trigger type (push, pull_request, etc.) is merged at the top level.
 */
function mergeOn(
  ...
  for (const [trigger, config] of Object.entries(incoming)) {
    ...
    // if both have the same trigger, keep existing (union means both are present)
  }
...
/**
 * Merge jobs by job id. Within a job, `steps` arrays are concatenated.
 */
function mergeJobs(
  existing: Record<string, unknown> | undefined,
  incoming: Record<string, unknown>

import { Inbox } from '@app/api-core/Modules/Triage/Inbox';
import { Schema } from 'effect';

const decodeInbox = Schema.decodeUnknownSync(Inbox);

/**
 * Pre-triaged showcase inbox for demo mode (no Postgres).
 * Mixed statuses so the product shell opens without the first-run gate.
 */
export const DemoInbox: Inbox = decodeInbox({
  summary: {
    processed: 5,
    handled: 2,
    needsAttention: 2,

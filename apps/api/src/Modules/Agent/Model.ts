 ...
 */
export const TriageModelLive: Layer.Layer<LanguageModel.LanguageModel> =
  OpenRouterLanguageModel.layer({
    model: MODEL,
    config: { reasoning: { effort: 'low' }, strictJsonSchema: true }
  }).pipe(Layer.provideMerge(IdGeneratorLive), Layer.provide(ClientLive));
 ...
 */
export const ToolModelLive: Layer.Layer<LanguageModel.LanguageModel> =
  OpenRouterLanguageModel.layer({
    model: MODEL,
